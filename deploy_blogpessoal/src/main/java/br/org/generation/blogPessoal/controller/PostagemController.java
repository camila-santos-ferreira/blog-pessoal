...
/* Indica que a classe é responsável por responder às requisições http enviadas para um endpoint (endereço) definido na anotação 
 * @RequestMapping
 */
@RestController
// Indica o endpoint (endereço) que a controladora responderá as requisições 
@RequestMapping("/postagens")
/* Indica que a classe controladora permitirá o recebimento de requisições realizadas de fora do domínio (localhost, em nosso caso) 
 * ao qual ela pertence. Essa anotação é essencial para que o front-end (Angular em nosso caso), tenha acesso à nossa API 
 * (O termo técnico é consumir a API). Esta anotação, além de liberar as origens, libera também os cabeçalhos das requisições.
 */
@CrossOrigin("*")
